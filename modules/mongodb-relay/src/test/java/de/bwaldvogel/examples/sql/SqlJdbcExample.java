...
package de.bwaldvogel.examples.sql;
 ...
 * <p>
 * Ignite nodes must be started in separate process using before running this example.
 */
public class SqlJdbcExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        IgniteJdbcThinDriver.register();

        // Open JDBC connection
        try (Connection conn = DriverManager.getConnection("jdbc:ignite:thin://127.0.0.1:10801?queryEngine=mongo&schema=SQL_PUBLIC_PERSON")) {
            ...
            try (Statement stmt = conn.createStatement()) {
                try (ResultSet rs =
                    stmt.executeQuery("SELECT p.name, c.name as city_name FROM Person p INNER JOIN City c on c.id = p.city_id")) {
                    print("Query results:");
