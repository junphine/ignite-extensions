...
public class ESQueryKernelIgniteHandler extends ESQueryHandler {
	...
	protected Object sendEsRequest(ESUpdate query, int index) throws Exception {
		...
		if(query.getAction().equals("_doc")) {			
			...
			if(query.getOp().equals(ESConstants.INSERT_FRAGMENT)){			
				...
				BinaryObject bobj = BinaryObjectUtil.jsonToBinaryObject(ignite,typeName,query.getQuery());
			...
			// TODO @byron
			for(JsonNode o: list) {
				BinaryObject bobj = BinaryObjectUtil.jsonToBinaryObject(ignite,typeName,(ObjectNode)o);
	...
	protected Object sendEsRequest(ESQuery query, int index) throws Exception {	
			...
			if(query.getAction().equals(ESConstants.ALL_FRAGMENT)) {	
				...
				docs.forEach(e->{
					if(!e.isNull() && e.has("_id")) {
						...
						Object _key = BinaryObjectUtil.jsonNodeToObject(e.get("_id"));
		...
		if(query.getAction().equals("_stats")) {			
			...
			ids.forEach(e->{
				if(!e.isNull()) {
					keys.add(BinaryObjectUtil.jsonNodeToObject(e));
				}
			});	
	...
	protected Object sendEsRequest(ESViewQuery query, int index) throws Exception {
        ...
        for(List<?> row: list) {
        	ObjectNode node = BinaryObjectUtil.getObjectNode(row, fieldsMeta);
			dataset.add(node); 
		...
		if(es1Response instanceof CacheQueryResult) {
			...
			while (es1Hits.hasNext() && hits.size() < limit) {
				...
				if(fieldsMeta.size()>0 && item instanceof List) {
					ObjectNode node = BinaryObjectUtil.getObjectNode((List)item, fieldsMeta);
					if (node!=null) {					
						addHit(hits, node);
					}
				}
