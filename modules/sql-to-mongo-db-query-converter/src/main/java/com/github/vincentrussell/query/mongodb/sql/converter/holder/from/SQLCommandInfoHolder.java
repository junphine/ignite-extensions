...
public final class SQLCommandInfoHolder implements SQLInfoHolder {
    ...
    public static final class Builder {
        ...
        public Builder setStatement(final Statement statement)
            ...
            } else if (Upsert.class.isAssignableFrom(statement.getClass())) {
                sqlCommandType = SQLCommandType.UPSERT;
                return setUpsert((Upsert) statement);
            } else {
                throw new ParseException("No supported sentence");
            }
        }
        ...
        /**
         * Set the insert information for this query if it is a update query.
         * @param insert the {@link Update} object
         * @return the builder
         * @throws com.github.vincentrussell.query.mongodb.sql.converter.ParseException if there is an issue
         * parsing the sql
         * @throws ParseException if there is an issue parsing the sql
         */
        public Builder setInsert(final Insert insert)
                throws com.github.vincentrussell.query.mongodb.sql.converter.ParseException, ParseException {
            SqlUtils.isTrue(insert.getTable() != null,
                    "there must be a table specified for insert");
            from = generateFromHolder(new FromHolder(this.defaultFieldType,
                    this.fieldNameToFieldTypeMapping), insert.getTable(), null);
            
            ArrayList<Column> columns = (ArrayList)insert.getColumns();
            ExpressionList list = (ExpressionList)insert.getItemsList();
            
            UpdateSet upset = new UpdateSet();
            upset.setColumns(columns);
            upset.add(list);
            updateSets.add(upset);
            return this;
        }
         ...
         * @throws ParseException if there is an issue parsing the sql
         */
        public Builder setUpsert(final Upsert update)
                throws com.github.vincentrussell.query.mongodb.sql.converter.ParseException, ParseException {
            ...
            ArrayList<Column> columns = (ArrayList)update.getColumns();
            ExpressionList list = (ExpressionList)update.getItemsList();
            
            UpdateSet upset = new UpdateSet();
            upset.setColumns(columns);
            upset.add(list);
            updateSets.add(upset);
            if(update.isUseDuplicate()) {
            	groupBys = new ArrayList<>();
            	for(Column col: update.getDuplicateUpdateColumns()) {
            		groupBys.add(col.getColumnName());
            	}
            }
            
            for(int i=0;i<columns.size();i++) {
            	if(columns.get(i).getColumnName().equals("_id") || columns.get(i).getColumnName().equals("id")) {
	            	Expression exp = list.getExpressions().get(i);           	
	            	whereClause = new EqualsTo(columns.get(i),exp);
	            	break;
            	}
            }
            ...
            if(whereClause==null) {
	            for(int i=0;i<columns.size();i++) {
	            	if(columns.get(i).getColumnName().equals("_key")) {
		            	Expression exp = list.getExpressions().get(i);          	
		            	whereClause = new EqualsTo(columns.get(i),exp);
		            	break;
	            	}
        ...
        public Builder setPlainSelect(final PlainSelect plainSelect)
            ...
            SqlUtils.isTrue(plainSelect != null && plainSelect.getFromItem() != null,
