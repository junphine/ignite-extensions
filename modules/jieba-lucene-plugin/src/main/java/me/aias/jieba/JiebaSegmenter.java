                ...
                if (sb.length() > 0) {
                    ...
                    if (mode == SegMode.SEARCH) {
                        ...
                        for (String token : sentenceProcess(sb.toString())) {
                        	...
                        	tokens.add(new SegToken(token, offset, offset + tokenLen));
                        	for (; j < tokenLen - 1; ++j) {
                        		if (tokenLen > 2) {                            
                                ...
                                if (tokenLen > 3 && j<tokenLen-2 ) {
                                    String gram3 = token.substring(j, j + 3);
                                    if (wordDict.containsWord(gram3))
                                        tokens.add(new SegToken(gram3, offset + j, offset + j + 3));
                                }
                                
                                if (tokenLen > 4 && j<tokenLen-3) {
                                    String gram4 = token.substring(j, j + 4);
                                    if (wordDict.containsWord(gram4))
                                        tokens.add(new SegToken(gram4, offset + j, offset + j + 4));
                                }
                        	}
                        	offset += token.length();
